...
public class ComboBox_kh extends JLabel {
	// 현재 콤보 수를 띄우는 라벨
	...
	public ComboBox_kh(DeathNotePlayerService playerService, Player player) {
	...
	class ComboNum extends JLabel implements Runnable {
		...
		public void run() {
			while (GameSelectFrame.isGameRunning()) {
				if (playerService.getScoreService_kh().getCombo() >= MAX_COMBO) {
					setIcon(comboNum[9]);
				} else {
					...
					case ONES:
						for (int i = 0; i < comboNum.length; i++) {
							if (playerService.getScoreService_kh().getCombo() % 10 == i) {
								setIcon(comboNum[i]);
							}
						}
						break;
					case TENS:
						for (int i = 0; i < comboNum.length; i++) {
							if ((playerService.getScoreService_kh().getCombo() % 100) / 10 == i) {
								setIcon(comboNum[i]);
							}
						}
						break;
					case HUNDREDS:
						for (int i = 0; i < comboNum.length; i++) {
							if ((playerService.getScoreService_kh().getCombo() % 1000) / 100 == i) {
								setIcon(comboNum[i]);
							}
						}
						break;
